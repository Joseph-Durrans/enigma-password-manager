...
int RotorSet::traverseForward(int pos) {
    for (int i = this->rotors.size() - 1; i >= 0; i--) {
        ...
        if (i != 0) {
            pos = posMod(pos - this->rotors[i]->position + this->rotors[i - 1]->position, this->rotors[i]->wiring.size());
        }
	}
...
std::string RotorSet::encode(std::string msg) {
    ...
    for (int i = 0; i < msg.size(); i++) {
        ...
        for (int j = 0; j < alpha.size(); j++) {
            if (alpha[j] == msg[i]) {
				pos = posMod(j + this->rotors[this->rotors.size() - 1]->position, alpha.size());
			}
		}
		...
		// Get the position of the character in the alphabet:
		pos = posMod(pos - this->rotors[this->rotors.size() - 1]->position, alpha.size());
...
int main() {
    ...
    for (Rotor* r : rs.rotors) {
        ...
        if (r->position > 25 || r->position < 0) {
            std::cout << "Error: rotor position must be between 0 and 25";
            return 0;
        }

        if (r->increment > 25 || r->increment < 0) {
